...
import { clusterLayer, clusterLayerSolo, initialHeatmapLayer } from './layers';
...
export function createInitialState(): UiState{
  return {
    ...
    heatmapLayer: initialHeatmapLayer,
    heatmap: false,
...
export class UiStore extends Store<UiState> {
  ...
  updateSelectedPheno(pheno){
    ...
    this.update(state => {
      ...
      if(pheno && pheno.title != 'ALL'){
        ...
        heatmapColor = JSON.parse(JSON.stringify(pheno.layer.paint['circle-color']));
        heatmapColor[2] = ['heatmap-density'];
      }
  ...
  setLayers(layers){
    this.update(state => ({...state, layers}));
  }
  ...
  updateDateRange(dateRange: Array<Date>){
    this.update(state => ({...state, dateRange}));
  }
