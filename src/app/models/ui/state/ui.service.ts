...
export class UiService {
  ...
  fetchGeocodeResults(searchstring) {
    ...
    return this.http.get('//locationiq.org/v1/search.php', {params}).pipe(first()).pipe(tap((res: any) => {
  ...
  setSearchTerm(searchTerm) {
    this.uiStore.update(state => ({...state, searchTerm}));
  }
  ...
  setClustering(clustering) {
    ...
    this.uiStore.update(state => ({...state, clustering}));
  }
  ...
  toggleHeatmap() {
    this.uiStore.update(state => {
        return ({
          ...state.heatmapLayer, layout:
            {
              ...state.baseLayer.layout,
              visibility: state.heatmap ? 'visible' : 'none'
            }
        });
      }
    );

    this.uiStore.update(state => ({...state, heatmap: !state.heatmap}));
  }
  ...
  setNumbers(numbers) {
    this.uiStore.update(state => ({...state, numbers}));
  }

  setCluster(cluster) {
    if (!cluster) {
      this.clusterTimeout = setTimeout(() => {
        this.uiStore.update(state => ({...state, cluster}));
      }, 250);
    } else {
  ...
  setInfoPheno(infoPheno) {
    this.uiStore.update(state => ({...state, infoPheno}));
  }

  setFilters(filters) {
    this.uiStore.update(state => ({...state, filters}));
  }
  ...
  setMapLoading(mapLoading) {
    this.uiStore.update(state => ({...state, mapLoading}));
  }

  setShowDateModal(showDateModal) {
    this.uiStore.update(state => ({...state, showDateModal}));
  }
  ...
  setFilterIds(ids) {
    this.uiStore.update(state => ({...state, filters: {...state.filters, ids}}));
  }
